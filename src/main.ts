        ...
        const attrs = new Proxy(
            {},
            {
                get(target, prop) {
                    ...
                    return dataView[types[p]](
                        row * row_offset + offsets[p],
                        true,
                    );
                },
            },
...
async function main() {
    ...
    const frame = (now) => {
        ...
        for (let gamepad of gamepads) {
            ...
            if(gamepad.buttons[12].pressed || gamepad.buttons[13].pressed){
                const p12 = gamepad.buttons[12].pressed as unknown as number;
                const p13 = gamepad.buttons[13].pressed as unknown as number;
                inv = translate4(inv, 0, -moveSpeed*(p12 - p13), 0);
                carousel = false;
            }

            if(gamepad.buttons[14].pressed || gamepad.buttons[15].pressed){
                const p14 = gamepad.buttons[14].pressed as unknown as number;
                const p15 = gamepad.buttons[15].pressed as unknown as number;
                inv = translate4(inv, -moveSpeed*(p14 - p15), 0, 0);
                carousel = false;
            }
    ...
    const selectFile = (file) => {
        ...
        if (/\.json$/i.test(file.name)) {
            ...
            stopLoading = true;
            fr.onload = () => {
                const r = fr.result as ArrayBuffer;
                splatData = new Uint8Array(r);
